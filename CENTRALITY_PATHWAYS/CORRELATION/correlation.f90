...
!   read(*,*) natom,nframe
    natom=454   ! number of atoms in the system
    nframe=100  ! number of frames in the trajectory
! static variables
   threenatom=3*natom
!*******************!
...
! Read the coordinates in xyz format
   ...
   open(1,file='CA.pdb') ! NAME OF TRAJ FILE
   do frame=1,nframe
      ii=1
      read(1,*)
      do i=1,natom
! read(1,*) char1,int2,char3 , char4, int5, coord(ii,frame), coord(ii+1,frame), coord(ii+2,frame)
         read(1,*) char1,int2,char3 , char4, char5, int5, coord(ii,frame), coord(ii+1,frame), coord(ii+2,frame)
  !       write(*,*) char1,int2,char3 , char4, int5, coord(ii,frame), coord(ii+1,frame), coord(ii+2,frame)
         ii=ii+3
!!!!!!!!!!!!!!!! ATOM      5  CA  MET A   1       3.414  14.928  -5.959  0.00  0.00    X
      enddo
   enddo
   close(1)
