...
!   read(*,*) natom,nframe
    ...
    do i=1,4
       ...
       if(algoc4.eq.'tres_d') treshold_dist=rvalue
    enddo
...
! Read the coordinates in xyz format
    ...
    open(1,file='./traj.pdb')     ! la trayectoria completa!

! First reading of the nheavyatom    
    ii=1
    jj=0
    read(1,*)
    read(1,*)
    read(1,*) 
    do i=1,Natom
        read(1,*) algoc4, atom_index, resname(i,1), resname(i,2), res_index(i), coord(ii), coord(ii+1), coord(ii+2)
        ii=ii+3
    ...
    read(1,*) 
        do j=1,natom
           if(indexheavy(j).ne.0) then
             read(1,*) algoc4, atom_index, resname(j,1), resname(j,2), res_index(jj), coord(ii), coord(ii+1), coord(ii+2)
             ii=ii+3
             jj=jj+1
           else
             read(1,*)
